...
public class AssetDetailActivity
        ...
        if (keyringService != null) {
            keyringService.getKeyringInfo(BraveWalletConstants.DEFAULT_KEYRING_ID, keyringInfo -> {
                if (keyringInfo != null) {
                    ...
                    WalletListItemModel thisAssetItemModel = new WalletListItemModel(R.drawable.ic_eth,
                                    mAsset.name, mAsset.symbol, mAsset.tokenId, "", "");
                    thisAssetItemModel.setBlockchainToken(mAsset);
                    Utils.setUpTransactionList(accountInfos, mAssetRatioService, mTxService, null,
                            null, thisAssetItemModel,
                            findViewById(R.id.rv_transactions), this, this,
    ...
    private void getBlockchainToken() {
        ...
        TokenUtils.getUserAssetsFiltered(mBraveWalletService, mChainId, TokenUtils.TokenType.ALL, (userAssets) -> {
            for (BlockchainToken userAsset : userAssets) {
                if (mChainId.equals(userAsset.chainId) && mAssetSymbol.equals(userAsset.symbol) && mAssetName.equals(userAsset.name) && (mAssetId.isEmpty() || mAssetId.equals(userAsset.tokenId)) && mContractAddress.equals(userAsset.contractAddress) && mAssetDecimals == userAsset.decimals) {
                    mAsset = userAsset;
                }
            }
