         ...
         network_url](const network::ResourceRequest& request) {
          ...
          if (request.method ==
            ...
            if (is_supports_interface_req) {
              EXPECT_NE(request_string.find(interface_id.substr(2)),
                        std::string::npos);
              ...
              VLOG(0) << "REQUEST STRING " << request_string
                      << " desired function_hash " << function_hash;
              EXPECT_NE(request_string.find(function_hash), std::string::npos);
