...
- Trigger saving of publisher icon as necessary (we need to define when that is)
- Rewrite uses of braveRewards.openBrowserActionUI and ShowActionUI in tests
- Implement |IsPublisherRegistered| and consider renaming "verified" field names
- Consider using a |ResizeObserver| for panel height tweaks instead of
  |NamedOverlay|
